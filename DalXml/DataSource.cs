...
internal class DataSource
{
    ...
    public List<DO.Order?> orderList { get; set; } = new List<DO.Order?>();//list of orders
    public List<DO.OrderItem?> orderItemList { get; set; } = new List<DO.OrderItem?>();//list of orderItems
    public List<DO.Product?> productList { get; set; } = new List<DO.Product?>();//list of products
    ...
    private void CreateOrderItem()//func to create a orderItems
    {
        for (int i = 0; i < 40; i++)
        {
            DO.Product? product = productList[randNum.Next(productList.Count)];
            orderItemList.Add(
                new DO.OrderItem
                {
