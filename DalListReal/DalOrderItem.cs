...
public class DalOrderItem : IOrderItem
{
   // DataSource _ds=DataSource.s_instance;//to access the data 
    ...
    { 
        if (oi.ID != 0 && DataSource.orderItemList.ElementAt(oi.ID).IsDeleted == true)//if the item already exists and was "deleted" => we came from the 'update' operation
        {
            int ind = 0;
            foreach (OrderItem orderi in DataSource.orderItemList)//go over OrderItem list
            {
                if (orderi.ID == oi.ID)//if oi exists in list
                    ind = DataSource.orderItemList.IndexOf(orderi);//save place of the matching existing OrderItem 
            }
            DataSource.orderItemList[ind] = oi;//place oi OrderItem in that place of ind
            return oi.ID;//return the id
        }
        else
        {
            if (oi.ID != 0 && DataSource.orderItemList.ElementAt(oi.ID).IsDeleted == false)//the item's ID exists, but it was not deleted from the collection => throw exression
                throw new Exception("Unothorized override");
            else
            {
                oi.ID = DataSource.Config.s_nextOrderItemNumber;//set an id number to OrderItem p
                DataSource.orderItemList?.Add(oi);//add oi to the OrderItem list
                return oi.ID;//return the id
            }
        } 
    }
    ...
    {
        OrderItem res = DataSource.orderItemList.Find(x => x.ID == id);
        if (res.ID != id)
    ...
    {
        int ind = 0;
        foreach (OrderItem orderi in DataSource.orderItemList)//gets the index
        {
            if (orderi.ID == id)//if found id in the OrderItem list
                ind = DataSource.orderItemList.IndexOf(orderi);//save index of that OrderItem
        }
        OrderItem oi = DataSource.orderItemList[ind];//oi is the OrderItem of that placement
        DataSource.orderItemList[ind] = oi; //updates "IsDeleted" to true in the OrderItem collection
    }

    public void Update(OrderItem oi)
    {
        bool flag = false;
        foreach (OrderItem it in DataSource.orderItemList)//go over OrderItem list
        {
        ...
        if (flag == true)//if found a matching id
        {
            ...
            foreach (OrderItem orderi in DataSource.orderItemList)//go over OrderItem list
            {
                if (orderi.ID == oi.ID)//if found a matching id to the one inputted
                    ind = DataSource.orderItemList.IndexOf(orderi);//save the index 
            }
            OrderItem ord = DataSource.orderItemList[ind];//ord is the OrderItem in ind index
