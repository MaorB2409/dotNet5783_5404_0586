...
internal static class DataSource
{
    ...
    static void s_Initialize()//creates new lists for the products etc
    {
        CreateProducts();//func to create a product
        CreateOrders();//func to create a order
        CreateOrderItem();//func to create a orderItems
    }
    private static void CreateProducts()//func to create a product 
    {
        ...
        for(int i=0;i<10;i++)
        {
            productList.Add(
                new()
                {
                    ...
                    Name = NameOfFurniture[randNum.Next(0, 7)],
                    Category = (Enums.Category)randNum.Next(6),
                    InStock = randNum.Next(19, 48)
                }) ;
        }

    }
    private static void CreateOrders()//func to create a order
    {
        ...
        for(int i=0;i<20;i++)
        {
            ...
            {
                ID=Config.NextProductNumber,
                CostumerName = customerName[randNum.Next(0,21)],
                CostumerEmail = customerEmail[randNum.Next(0,21)],
                CostumerAddress = address[randNum.Next(0,21)],
                OrderDate=DateTime.Now-new TimeSpan(randNum.NextInt64(10L*1000L*3600L*24L*100L)),
            };
            myOrder.OrderDate=DateTime.Now-new TimeSpan();//
            myOrder.ShipDate=myOrder.OrderDate-new TimeSpan(randNum.NextInt64(10L*1000L*3600L*24L*100L));//
            myOrder.DeliveryDate=myOrder.ShipDate-new TimeSpan(randNum.NextInt64());//
            orderList.Add(myOrder);//add the order to the list
        }

    }
    private static void CreateOrderItem()//func to create a orderItems
    {
      for(int i=0;i<40;i++)
      {
